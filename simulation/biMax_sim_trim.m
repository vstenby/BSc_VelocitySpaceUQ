%% Bi-Maxwellian Distribution Simulation (Different trimming)
function biMax_sim_trim(sim_idx)
    ...
    % - - - - - - - - - - Simulation setup  - - - - - - - - - - - - - - - -
    % In this simulation, I explore the difference that simulation makes.
    
    keeps = [1 5 10 15];
    keep = keeps(sim_idx); clear keeps;
    ...
    %Do the sampling.
    [xNNHGS0, alphasim0, deltasim0, lambdasim0, NNHGS0info] = NNHGS(A,b,[],nsim,'solver','lsqnonneg', 'welford',true,'keep', keep, 'nburnin',nburnin);
    [xNNHGS1, alphasim1, deltasim1, lambdasim1, NNHGS1info] = NNHGS(A,b,L,nsim,'solver','lsqnonneg','welford',true,'keep', keep, 'nburnin',nburnin);
    ...
    outputpath = sprintf('./biMax_sim_trim/trim_%02d.mat',nsim);
    save(outputpath)
end